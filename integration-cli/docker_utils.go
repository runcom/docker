...
func setupRegistryAt(c *check.C, url string, schema1 bool) *testRegistryV2 {
	testRequires(c, RegistryHosting)
	reg, err := newTestRegistryV2At(c, url, schema1)
	c.Assert(err, check.IsNil)
...
func setupRegistry(c *check.C, schema1 bool) *testRegistryV2 {
	return setupRegistryAt(c, privateRegistryURL, schema1)
}
