...
const NOLOGINUID = 0xffffffff // -1 means no login user
...
func getLoginUID(ucred *syscall.Ucred, fd int) (uint32, error) {
	...
	loginuidInt, err := strconv.ParseUint(string(loginuid), 10, 64)
	if err != nil {
		logrus.Errorf("Failed to convert loginuid to int: %v", err)
		return NOLOGINUID, err
	}
	if loginuidInt < 0 || loginuidInt > NOLOGINUID {
		logrus.Errorf("LoginUID %s is out of valid range", string(loginuid))
		return NOLOGINUID, nil
	} else {
...
func (s *Server) LogAction(w http.ResponseWriter, r *http.Request) error {
	...
	default:
		...
		message = fmt.Sprintf("PID=%d", ucred.Pid) + message
		...
		message = fmt.Sprintf("LoginUID=%d, %s", loginuid, message)
